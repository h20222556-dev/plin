...
export function RecordsProvider({ children }) {
  ...
  const fetchRecords = useCallback(async (isSilent = false) => {
    ...
    try {
      let query = supabase.from(TABLE_NAME).select('id, user_id, concert_name, artist, date, venue, lat, lng, pin_icon, memo, weather, setlist, tags, photos, is_public, created_at');
