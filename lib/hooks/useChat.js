...
export function useChat(roomId, recipientId) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      if (isDemoMode) {
        // 데모 모드: 데모 유저 그룹 내에서 오간 메시지만
        query = query.or(`sender_id.in.(${DEMO_USER_IDS.join(',')}),receiver_id.in.(${DEMO_USER_IDS.join(',')})`);
      } else {
        // 일반 모드: 데모 유저들의 모든 메시지 제외
        query = query.not('sender_id', 'in', `(${DEMO_USER_IDS.join(',')})`).not('receiver_id', 'in', `(${DEMO_USER_IDS.join(',')})`);
      }
