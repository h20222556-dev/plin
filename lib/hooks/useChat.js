...
export function useChat(roomId, recipientId) {
  ...
  const { user, isDemoMode, DEMO_USER_ID } = useAuth();

  const fetchMessages = useCallback(async () => {
    if (!user || !roomId) return;
    setLoading(true);
    try {
      let query = supabase.from('chats').select('*').eq('room_id', roomId);
      
      if (isDemoMode) {
        // 데모 모드: 데모 유저가 보냈거나 받은 메시지만 (보안상 한 번 더 체크)
        query = query.or(`sender_id.eq.${DEMO_USER_ID},receiver_id.eq.${DEMO_USER_ID}`);
      } else {
        // 일반 모드: 데모 유저 메시지 제외
        query = query.neq('sender_id', DEMO_USER_ID).neq('receiver_id', DEMO_USER_ID);
      }
  ...
  const sendMessage = async (text) => {
    if (isDemoMode) {
      alert('데모 모드에서는 메시지를 보낼 수 없습니다. 로그인 후 이용해주세요.');
      return;
    }
