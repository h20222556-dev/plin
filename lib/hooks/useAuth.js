...
export function AuthProvider({ children }) {
      ...
      if (data) {
        ...
        if (data.nickname === '새로운 유저' && googleName) {
           const { data: updatedUser } = await supabase
            ...
            .select()
            .single();
           setUser(extUser(updatedUser || data));
        } else {
          setUser(extUser(data));
        }
      } else {
        // 프로필이 없는 경우 새로 생성
  ...
  const updateProfile = async (updates) => {
    ...
    try {
      ...
      setUser(prev => ({
        ...
        user_metadata: prev?.user_metadata || {},
      }));
    } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ user, setUser, loading, login, signup, logout, loginWithGoogle, updateProfile }}>
      {children}
