...
export function useCommunity() {
  ...
  const fetchPosts = useCallback(async () => {
    setLoading(true);
      console.log('[DEBUG] fetchPosts 호출됨, isDemoMode:', isDemoMode);
      console.log('[DEBUG] DEMO_USER_IDS:', DEMO_USER_IDS);
      ...
      // [임시] 필터 전 전체 데이터 확인
      const { data: testData, error: testErr } = await query;
      console.log('[DEBUG] 필터 전 전체 데이터:', testData);
      console.log('[DEBUG] 필터 전 에러:', testErr);

      if (isDemoMode) {
        query = query.in('user_id', DEMO_USER_IDS);
      } else {
        const demoIdsStr = `(${DEMO_USER_IDS.join(',')})`;
        query = query.not('user_id', 'in', demoIdsStr);
      }

      const { data, error } = await query.order('created_at', { ascending: false });
      
      console.log('[DEBUG] 필터 적용 후 데이터:', data);
      console.log('[DEBUG] 필터 적용 후 에러:', error);
